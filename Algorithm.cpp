...
cell_info update_walls(int angle_now, int row, int col) {
  ...
  new_cell.walls[UP] = wallFront();
  new_cell.walls[DOWN] = 0;
  new_cell.walls[LEFT] = wallLeft();
  new_cell.walls[RIGHT] = wallRight();
  new_cell.dead = 0;
  new_cell.visited = 1;
...
std::queue<std::pair<int, int>> shorted_path_go(std::vector<std::vector<int>> &arr, int angle_now, coord start, coord dest)  // vẽ ra con đường ngắn nhất
{
  std::queue<int> next_dir_path;
  std::queue<std::pair<int, int>> shortest_path;
...
void exec_shortest_path(std::queue<std::pair<int, int>> shortest_path, int &cur_direction) {
...
int main(int argc, char *argv[]) {
  ...
  statemachine(robot_commands);  // Khi ấn nút sẽ chạy func này
  return 0;
}
